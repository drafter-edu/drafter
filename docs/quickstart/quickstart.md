# Drafter Quick Start Guide {#quickstart}
...
![Simple website with the text \"Hello World!\" and \"Welcome to Drafter\" along with some more information further below.](quickstart_empty_site.png){width="700px"}
...
``` {.python emphasize-lines="2-6" linenos=""}
from drafter import *
...
``` {.python emphasize-lines="8-13" linenos=""}
from drafter import *
from dataclasses import dataclass
...
![Simple website with the text \"The message is\" and \"The original message\" along with some more information further below.](quickstart_simple_state.png){width="700px"}

## 1. Another Route

We can see the state now, but the state is *static* - nothing ever changes. Let\'s add in another page that changes the state.

``` {.python emphasize-lines="15-21" linenos=""}
from drafter import *
from dataclasses import dataclass
...
``` {.python emphasize-lines="13" linenos=""}
from drafter import *
from dataclasses import dataclass
...
![Clickable button is now visible on the original page](quickstart_button_state_1.png){width="700px"}

Once we click the button, we are taken to the new page.

![Clickable button was added on the first page that leads to this new page.](quickstart_button_state_2.png){width="700px"}
...
``` {.python emphasize-lines="22-23" linenos=""}
from drafter import *
from dataclasses import dataclass
...
``` {.python emphasize-lines="24, 27-30" linenos=""}
from drafter import *
from dataclasses import dataclass
...
![A textfield with a save button next to it](quickstart_string_state.png){width="700px"}
...
``` {.python emphasize-lines="14-15, 34-39" linenos=""}
from drafter import *
from dataclasses import dataclass
...
``` {.python emphasize-lines="7, 15-16, 38-43, 45" linenos=""}
from drafter import *
from dataclasses import dataclass
...
``` {.python emphasize-lines="3, 50-70" linenos=""}
from drafter import *
from dataclasses import dataclass
...
![The page content expandable](quickstart_copy_page_content.png){width="700px"}
...
2.  Add another page with a numeric field (hint: you will need to check that the user entered text [isnumeric]{.title-ref} and then convert it to an integer).
3.  Use the [SelectBox]{.title-ref} to make a dropdown list with several options.
4.  Use the [NumberedList]{.title-ref} or [BulletedList]{.title-ref} to represent a list of values, and then use the [append]{.title-ref} method to add a new item via a button.

Check out the [documentation of Components <fulldocs>] to see all that you can add to your website!

And if you want a longer example, check out the [calculator example <calculator>] to see a more complicated web application.
