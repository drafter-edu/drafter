...
| `favicon`       | `str`  | The small browser-tab icon: a URL or the path to an image file next to your program.                                                                                   |
...
`start_server` are applied last, so they take precedence if you
accidentally use both.

## Notes

- **Nothing after the call runs.** `start_server` takes over the
  program, so all configuration must come before it; code placed
