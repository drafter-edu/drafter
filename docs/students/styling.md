...
-   Disabling the default theme (`set_website_style("none")`: [changing themes](#changing-themes)
-   Adding custom CSS to the website (`add_website_css`): [Adding Website CSS](#adding-website-css)
-   Using CSS classes to style components (`classes` keyword parameter): [CSS Classes](#css-classes)
-   Using styling functions to style components (e.g., `bold`): [Styling Functions](#styling-functions)
-   Using keyword parameters to style components (e.g., `style_color` and `style_float`): [Keyword Parameters](#keyword-parameters)
-   Embedding a CSS style tag in a page (`PAGE_SPECIFIC_STYLE`): [CSS Style Tags](#css-style-tags)
...
The above example shows how to use the `float_right` function to make the button align on the right hand side of the screen.
...
Changes the text color. The color can be a named color (e.g. \"red\") or a hex code (e.g. \"#FF0000\"). See the [colors] page for available HTML colors.

param color

:   The color to change the text to.
...
Changes the background color of the component (the `background-color` CSS attribute). The color can be a named color (e.g. \"red\") or a hex code (e.g. \"#FF0000\"). See the [colors] page for available HTML colors.
...
:   The size to change the text to.
...
Changes the font of the text. The font must be a valid font name (e.g. `"Arial"`, `"Times New Roman"`, `"Courier New"`). See the [fonts] page for available fonts.

param font

:   The font to change the text to.
...
:   The alignment to change the text to.
...
:   The decoration to change the text to.
...
:   The transformation to change the text to.
...
:   The margin to change the component to.
...
Additionally, a border can have a width and color. The width must be a string followed by the units (e.g. \"1px\") or an integer (e.g. 1). The color can be a named color (e.g. \"red\") or a hex code (e.g. \"#FF0000\"); see the [colors] page for available HTML colors. The format is `"style width color"`. For example:
...
:   The border to change the component to.
...
:   The padding to change the component to.
...
:   The width to change the component to.
...
:   The height to change the component to.
...
Another way to style components is to use keyword parameters. Any component can take [style\_]{.title-ref} prefixed keyword parameters. For example:
...
You can narrow down the styling by using CSS classes. You can add a class to any component using the `classes` keyword parameter. You can then use the class name in your CSS style.
...
If you are using the default theme (`skeleton`), then you may need to be more specific with your CSS selectors. For example, if you want to style a button, you may need to use the following CSS:
...
Otherwise, the default theme may override your styles. You can disable the default theme by using `set_website_style("none")` (see [changing themes](#changing-themes)).

## Adding Website CSS

If you don\'t want to have to include the `STYLE` constant in every page, you can use the `add_website_css` function to add the CSS to every page on the website:

::: function
add_website_css(css: str) add_website_css(selector: str, css: str)
...
:   The CSS selector to apply the CSS to, or the CSS content if the second parameter is None.
...
:   The CSS content to apply to the selector.
...
This can be placed anywhere before the `start_server` function is called, but we recommend putting it directly after your imports for clarity.

## Changing Themes {#changing themes}

Drafter ships with a default theme called `skeleton`. You can change the theme by using the `set_website_style` function. There are currently a few different options.
...
:   The theme to set the website style to.
