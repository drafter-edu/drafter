...
class Client:
    ...
    def goto(self, url: str, form_data: Any = None, action = "system"):
        if self.navigation_func is None:
            raise RuntimeError("Navigation function not set in Client.")
        return self.initiate_request(url, form_data, True, action)
    
    def navigate(self, nav_event: NavEvent):
        return self.initiate_request(nav_event.url, nav_event.data, True, nav_event.kind)
    ...
    def make_request(self, url: str, form_data: Any = None, action: str = "submit") -> Request:
        ...
        if is_pyodide():
            raise NotImplementedError("Async request handling in Pyodide not implemented yet.")
        elif is_skulpt():
    ...
    def initiate_request(self, url: str, form_data: Any = None, remember = True, action = "submit"):
        if self.navigation_func is None:
            raise RuntimeError("Navigation function not set in Client.")
        debug_log("client.initiate_request", url, form_data, remember, action)
        request = self.make_request(url, form_data, action)
        if remember:
    ...
    def mount_navigation(self, root: Any, on_navigation: Callable):
        ...
        def handle_click(event: Any):
            ...
            if nearest_nav_link and root.contains(nearest_nav_link):
                ...
                if form:
                    ...
                    if is_pyodide():
                        form_data = js.FormData.new(form, None if is_anchor else nearest_nav_link)
                    elif is_skulpt():
                        form_data = js.FormData(form, None if is_anchor else nearest_nav_link)
                    else:
                        raise RuntimeError("Unknown web runtime.")
                    nav_event = NavEvent(kind="link", url=name, data=form_data, submitter=nearest_nav_link)
        ...
        def submit_handler(event: Any):
            ...
            nav_event = NavEvent(kind="form", url=url, data=form_data, submitter=submitter)
            return on_navigation(nav_event)
    ...
    def update_site(self, response: Response, callback: Callable[[Request], Response]) -> bool:
        ...
        if body is not None:
            ...
            mounted = self.mount_navigation(element, self.navigate)
    ...
    def handle_popstate(self, event: Any) -> None:
        ...
        if event and "state" in event and "request_id" in event.state and event.state["request_id"] is not None:
            ...
            self.initiate_request(url, None, False, "back")
        else:
            ...
            self.initiate_request("index", None, False, "back")
