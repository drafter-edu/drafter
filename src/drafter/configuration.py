...
def configure_system(
    ...
    if from_cli:
        original_arguments = sys.argv[
            1:
        ]  # Exclude the script name for non-CLI contexts
    else:
        original_arguments = sys.argv[:]
    ...
    bootstrap_env_vars = BootstrapConfiguration.parse_env_variables(
    ...
    if not is_web() and bootstrap_config.path is None:
        ...
        if len(original_arguments) > 0 and not is_placeholder_script_argument(
            ...
            print(
                "Warning: No entry path provided via command line arguments or environment variables. Drafter may not function correctly without an entry point."
            )
