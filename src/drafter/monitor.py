...
class Monitor:
    ...
    def track_response(
        self, response: Response, state_snapshot: Any = None
    ) -> None:
        """
    ...
    def generate_debug_html(
        ...
        try:
            snapshot = self.get_snapshot(current_state, initial_state, routes, server_config)
    ...
    def _render_state_section(self, state: Any) -> List[str]:
        ...
        try:
            ...
            parts.append(f"<p class='error'>Failed to render state: {html.escape(str(e))}</p>")
    ...
    def _render_visit(self, index: int, visit: PageVisitTelemetry) -> List[str]:
        ...
        if visit.duration_ms is not None:
            parts.append(f"<span class='visit-duration'>{visit.duration_ms:.1f}ms</span>")
        
        if visit.response:
            status_class = "status-ok" if visit.response.status_code < 400 else "status-error"
            parts.append(f"<span class='visit-status {status_class}'>{visit.response.status_code}</span>")
    ...
    def _render_routes_section(self, routes: Dict[str, Any]) -> List[str]:
        ...
        for url, func in sorted(routes.items()):
            ...
            parts.append(f"<li><code>{html.escape(url)}</code> → {html.escape(func_name)}</li>")
