...
from drafter.scaffolding.utils import copy_assets_to
...
def _build_environment_js(variables: List[tuple[str, Any]]) -> str:
    ...
    for key, value in variables:
        if isinstance(value, str):
            ...
            raise ValueError(
                f"Unsupported environment variable type: {type(value)} for {key}"
            )
...
def _build_skulpt_deployment(
    ...
    for filename in additional_files:
        if not filename.exists():
            console.print(
                f"[yellow]Warning:[/yellow] Additional file {filename} does not exist - skipping."
            )
            continue
        try:
            ...
            js_lines.append(
                SK_TEMPLATE_LINE.format(filename=str(filename), content=content)
            )
            console.print(f"• Adding additional file: {filename}")
        except Exception as e:
            console.print(
                f"[yellow]Warning:[/yellow] Failed to read file {filename} - skipping. Error: {e}"
            )
