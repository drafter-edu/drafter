...
- **url**: The URL to connect to the function as a route. Defaults to function's name, if not given.
  - Type: `str`
...
- **state**: The new value of the page, hidden from the user. This is usually an instance of a dataclass.
        If there is no change to the state, then will just be the `state` parameter.
- **content**: The actual content of the page, eventually rendered as HTML in the browser. Can combine both
...
### `start_server()`
    ...
    usually defaulting to [http://localhost:8080/ ](http://localhost:8080/). No code will be executed
    after this call.
...
- **text**: The text to display for the link.
  - Type: `str`
- **url**: Either a string representing the full URL you are linking to (possibly external to the server) or the
...
- **name**: The name of the text box, which will be used to identify the text box when the user submits the form.
  ...
  - Type: `str`
- **default_value**: The initial value of the text box, which will be displayed to the user. Defaults to `None`,
...
- **name**: The name of the text area, which will be used to identify the text area when the user submits the form.
                 Make sure this is a valid Python identifier (start with a letter followed only by letters, numerals,
                 and underscores). This must match the same parameter name in the corresponding linked route.
  - Type: `str`
- **default_value**: The initial value of the text area, which will be displayed to the user. Defaults to `None`,
...
- **name**: The name of the select box, which will be used to identify the select box when the user submits the form.
                 Make sure this is a valid Python identifier (start with a letter followed only by letters, numerals,
                 and underscores). This must match the same parameter name in the corresponding linked route.
  - Type: `str`
- **options**: The list of options to display in the select box. Each option should be a string.
  - Type: `list[str]`
- **default_value**: The initial value of the select box, which will be displayed to the user. Defaults to `None`,
...
- **name**: The name of the check box, which will be used to identify the check box when the user submits the form.
                 Make sure this is a valid Python identifier (start with a letter followed only by letters, numerals,
                 and underscores). This must match the same parameter name in the corresponding linked route.
  - Type: `str`
- **default_value**: The initial value of the check box, which will be displayed to the user. Defaults to `None`,
...
- **text**: The text to display for the button.
  - Type: `str`
- **url**: Either a string representing the full URL you are linking to (possibly external to the server) or the
                name of a function that is a route on the server (without quotes or calling parentheses).
  - Type: `str or function`
- **arguments**: Any additional arguments to pass to the server when the button is pressed. These should be
...
- **items**: The list of items to display in the ordered list. Each item should be a string. If they are not
...
- **items**: The list of items to display in the unordered list. Each item should be a string. If they are not
                  strings, then they will be converted using the builtin `str` function.
  - Type: `list[str]`
...
- **body**: The text to display in the header.
  - Type: `str`
- **level**: The size of the header, with 1 being the largest and 6 being the smallest. Defaults to 1.
  - Type: `int`
...
- **data**: The data to display in the table. Each row should be a list of strings, list of dataclass instances,
                 or a single dataclass instance.
  - Type: `list[list[str]] or list[object] or object`
...
- **components**: The components to display in the span. Each component should be a string or a `Component`
...
- **name**: The name of the argument, which will be used to identify the argument when the user submits the form.
             Make sure this is a valid Python identifier (start with a letter followed only by letters, numerals,
             and underscores).
  - Type: `str`
- **value**: The value of the argument, which will be passed to the server. This can be a string, integer, float,
              or boolean.
...
- **filename**: The name of the file that will be downloaded.
  - Type: `str`
- **contents**: The contents of the file that will be downloaded. This should be a string.
  - Type: `str`
- **content_type**: The MIME type of the file. Defaults to ``text/plain``.
  - Type: `str`
...
- **components**: The components to display in the division or row. Each component should be a string or a `Component`
                       instance. You do not pass them in as a list, but as separate arguments (like the `print`)
                       function.
  - Type: `str | Component`
...
- **text**: The text to display in the preformatted text. This will be rendered in a monospaced font.
  - Type: `str`
...
- **name**: The name of the file upload component, which will be used to identify the file when the user submits the form.
                 Make sure this is a valid Python identifier (start with a letter followed only by letters, numerals,
                 and underscores). This must match the same parameter name in the corresponding linked route.
  - Type: `str`
- **accept**: The file types that the user can upload. This should be a comma-separated list of MIME types.
