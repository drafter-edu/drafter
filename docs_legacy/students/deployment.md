...
If you have other files besides `main.py`, then check the appendix at the bottom of this document for how to upload them: [appendix_upload_files](deployment.md#appendix-uploading-files).
...
!!! note
    If you want to record the deployed version of your website, you can skip this step and record the video after you have deployed your website (see step [view_deployment](deployment.md#view-the-deployment) ) . Just make sure to record the video before the deadline.
...
* The planning document that you created, provided as the filename (see [planning_document](deployment.md#upload-the-planning-document) above).
* The URL (address) of the publicly-accessible video (see [video](deployment.md#record-a-video) above) that you uploaded, replacing the existing text inside of the angle brackets (`<https://my.video.com/>`) with the URL of your video.
...
Once you have the deployed URL, you can skip down to the last step to submit on Canvas (see [submit_on_canvas](deployment.md#submit-on-canvas)).
...
You can also get more details about the deployment from the deployment dashboard (see [deployment_dashboard](deployment.md#deployment-dashboard)).
