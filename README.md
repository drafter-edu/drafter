...
| `--load-packages-automatically` | on                            | Automatically load Python packages detected in the student's code, in addition to `--system-packages` / `--project-packages`. Disable with `--no-load-packages-automatically` to load only the explicitly listed packages. |
