...
def load_file_page(state: State):
    try:
        with open(state.filename) as f:
            file_content = f.read()
    except FileNotFoundError:
        file_content = "*File not found*."
    return Page(state, ["Actual file content:", file_content, Button("Go back", index)])
...
start_server(
    State(filename=DEFAULT_FILENAME, file_content=initial_content),
