...
16. **Hosting layout**: gallery apps on the same GH Pages site under `/your-project/gallery/…` (recommended, same-origin simplicity) or a separate deployment?
17. **Guided-project lineup**: approve the four designed projects (virtual pet, story maker, quiz game, finishing) or substitute equivalent domains that preserve the concept thread (§5.2); this decision confirms retiring cookie/bank/adventure/store as standalone tutorials.
18. **Archetype list**: approve the eight project archetypes bounding final-project choice (§5.4), and whether any course-specific archetypes should be added or removed.
